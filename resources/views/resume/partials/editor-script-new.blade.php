...
(() => {
    ...
    if (!app) return;
    const state = Object.assign({name:'', email:'', mobile:'', location:'', social_links:[], contact:'', address:'', summary:'', skills:[], experience:[{company:'', role:'', points:['']}], education:[''], projects:[]}, JSON.parse(app.dataset.initial || '{}'));
    ...
    function renderBasicPreview() {
        syncLegacyContact();
        $('cv-preview').innerHTML = `<header class="border-b-2 border-gray-950 pb-4"><h1 class="text-3xl font-bold uppercase text-gray-950">${esc(state.name || 'Your Name')}</h1><p class="mt-2 text-sm text-gray-600">${esc([state.email, state.mobile, state.location].filter(Boolean).join(' | '))}</p><p class="text-sm text-gray-600">${esc((state.social_links || []).join(' | '))}</p></header><section class="mt-6"><h2 class="text-xs font-bold uppercase text-teal-700">Summary</h2><p class="mt-2 text-sm leading-6 text-gray-700">${esc(state.summary)}</p></section><section class="mt-6"><h2 class="text-xs font-bold uppercase text-teal-700">Skills</h2><p class="mt-2 text-sm text-gray-700">${esc((state.skills || []).join(', '))}</p></section><section class="mt-6"><h2 class="text-xs font-bold uppercase text-teal-700">Experience</h2>${(state.experience || []).map(e => `<div class="mt-3"><h3 class="font-bold text-gray-950">${esc(e.role)}</h3><p class="text-sm text-gray-500">${esc(e.company)}</p><ul class="mt-2 list-disc pl-5 text-sm text-gray-700">${(e.points || []).map(p => `<li>${esc(p)}</li>`).join('')}</ul></div>`).join('')}</section><section class="mt-6"><h2 class="text-xs font-bold uppercase text-teal-700">Education</h2><ul class="mt-2 list-disc pl-5 text-sm text-gray-700">${(state.education || []).filter(Boolean).map(e => `<li>${esc(e)}</li>`).join('')}</ul></section>`;
    }
    ...
    document.querySelectorAll('.cv-field').forEach(input => input.addEventListener('input', e => { const f=e.target.dataset.field; state[f] = ['skills','social_links'].includes(f) ? list(e.target.value) : e.target.value; renderTemplatePreview(); }));
