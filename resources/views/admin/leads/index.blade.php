...
<div class="p-6 lg:p-8">
    <div class="mb-6 flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
        <div>
            <p class="text-xs font-bold uppercase tracking-widest text-blue-600">Enquiry Management</p>
            <h1 class="mt-2 text-2xl font-bold text-slate-950">General Enquiries</h1>
            <p class="mt-1 text-sm text-slate-500">Messages submitted from the public contact form. Visible to all team members with Team access.</p>
        </div>
        <form method="GET" class="flex w-full gap-2 sm:w-auto">
            <input name="q" value="{{ $search }}" placeholder="Search enquiries" class="min-w-0 flex-1 rounded-lg border-slate-300 text-sm focus:border-blue-600 focus:ring-blue-600 sm:w-72">
                ...
                <tbody class="divide-y divide-slate-100">
                    @forelse($leads as $lead)
                        <tr>
                            ...
                            <td class="px-4 py-4 text-sm text-slate-700">{{ $lead->subject ?: 'General enquiry' }}</td>
                            ...
                            <td class="px-4 py-4">
                                <div class="flex justify-end gap-2">
                                    ...
                                    <form method="POST" action="{{ route('admin.leads.destroy', $lead) }}" onsubmit="return confirm('Delete this general enquiry?')">
                                        @csrf
                                        @method('DELETE')
                    ...
                    @empty
                        <tr><td colspan="4" class="px-4 py-10 text-center text-sm text-slate-500">No general enquiries found.</td></tr>
                    @endforelse
                </tbody>
            </table>
        </div>
